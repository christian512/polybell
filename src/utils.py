...
def check_equiv_bell(bell1, bell2, allowed_perms, dets, tol=1e-6):
    """
    Checks if two bell inequalities are equivalent under relabelling
    ...
    if check_diff_repr_same_ineq(bell1, bell2, dets):
        return True
    # iterate over all allowed permutations
    for i in range(len(allowed_perms)):
        # check if the two bell inequalities are equal
        if np.sum((bell1-bell2[allowed_perms[i]])**2) < tol**2:
            return True

    return False

def check_equiv_bell_encapsulated(bell1, bell2, allowed_perms, dets, tol=1e-6):
    """
