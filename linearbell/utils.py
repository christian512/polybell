...
def get_relabelling_generators_new(inputs_a, inputs_b, outputs_a, outputs_b):
...
def equiv_check_adjacency_panda(bell1, bell2, relabels, dets, tol=1e-6):
    """ Equivalence checking for faces generated by PANDA (i.e. with RHS included in the face) """
    # rescale face to form b * det <= 0
    n = np.sum(dets[0])
    bell1 = bell1[:-1] + (bell1[-1] + 1) / n
    bell2 = bell2[:-1] + (bell2[-1] + 1) / n
    return equiv_check_adjacency_testing(bell1, bell2, relabels, dets, tol=tol)
